...
/** An InstructionTag object represents an HTML tag, such as <b> or </table>. */
public class InstructionTag {
    ...
    public boolean equals(Object o) {
        if (o instanceof InstructionTag) {
            InstructionTag other = (InstructionTag) o;
    ...
    public boolean matches(InstructionTag other) {
    ...
    public static InstructionTag parse(String tagText) {
        ...
        return new InstructionTag(element, isOpenTag);
    }
    ...
    public static LinkedList<InstructionTag> tokenize(String text) {
        StringBuffer buf = new StringBuffer(text);
        LinkedList<InstructionTag> queue = new LinkedList<InstructionTag>();

        while (true) {
            InstructionTag nextTag = nextTag(buf);
            if (nextTag == null) {
                break;
            } else {
                queue.add(nextTag);
    ...
    private static InstructionTag nextTag(StringBuffer buf) {
    ...
    public static void main(String[] args) {
		LinkedList<InstructionTag> list = InstructionTag.tokenize("1.  This is called <B> <U> <I> Frame </I> <U> </B> >");
		for (InstructionTag htmlTag : list) {
			System.out.println(htmlTag.toString());
		}
	}
	*/
}
